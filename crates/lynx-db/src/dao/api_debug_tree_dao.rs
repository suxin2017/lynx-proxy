...
impl ApiDebugTreeDao {
    ...
    async fn handle_cross_parent_move(
        ...
        self.adjust_sort_order_after_removal(txn, original_parent_id, original_sort_order)
            .await?;

        // 2. 确定在目标父节点中的排序位置
        let target_sort_order = if let Some(order) = new_sort_order {
            // 在目标父节点中为新节点腾出位置
            self.adjust_sort_order_for_insertion(txn, target_parent_id, order)
                .await?;
            order
        } else {
            // 如果没有指定排序，则放到目标父节点的最后
            self.get_max_sort_order_in_txn(txn, target_parent_id)
                .await?
                + 1
        };
    ...
    async fn handle_same_parent_move(
        ...
        if original_sort_order < new_sort_order {
            ...
            self.shift_nodes_forward(txn, parent_id, original_sort_order + 1, new_sort_order)
                .await?;
        } else {
            // 向前移动：将新位置到原位置之间的节点向后移动
            self.shift_nodes_backward(txn, parent_id, new_sort_order, original_sort_order - 1)
                .await?;
        }
    ...
    async fn adjust_sort_order_after_removal(
        ...
        query
            ...
            .col_expr(
                api_debug_tree::Column::SortOrder,
                Expr::col(api_debug_tree::Column::SortOrder).sub(1),
            )
            .col_expr(
                api_debug_tree::Column::UpdatedAt,
    ...
    async fn adjust_sort_order_for_insertion(
        ...
        query
            ...
            .col_expr(
                api_debug_tree::Column::SortOrder,
                Expr::col(api_debug_tree::Column::SortOrder).add(1),
            )
            .col_expr(
                api_debug_tree::Column::UpdatedAt,
                Expr::value(chrono::Utc::now().timestamp()),
...
mod tests {
    ...
    use crate::entities::api_debug::{HttpMethod, RequestType};
    ...
    async fn test_move_node_cross_parent() {
        ...
        // 在文件夹1中创建多个请求
        let _request1 = dao
            .create_request_node(CreateRequestNodeRequest {
                name: "请求1".to_string(),
    ...
    async fn test_rename_node() {
        ...
        let renamed_node = dao
            .rename_node(folder.id, rename_req)
            .await
            .unwrap()
            .unwrap();
    ...
    async fn test_reorder_nodes() {
        ...
        // 重新排序：将顺序改为 3, 1, 2
        let reorder_data = vec![(child3.id, 1), (child1.id, 2), (child2.id, 3)];

        dao.reorder_nodes(Some(parent.id), reorder_data)
            .await
            .unwrap();
    ...
    async fn test_create_real_api_debug_with_tree() {
        ...
        // 创建真实的API debug记录
        let login_api = api_debug_dao
            .create(CreateApiDebugRequest {
                name: "用户登录".to_string(),
                ...
                body: Some(
                    r#"{"username": "test@example.com", "password": "password123"}"#.to_string(),
                ),
                timeout: Some(30),
                is_history: false,
            })
        ...
        let get_profile_api = api_debug_dao
            .create(CreateApiDebugRequest {
        ...
        let update_profile_api = api_debug_dao
            .create(CreateApiDebugRequest {
                ...
                request_type: RequestType::Http,
                body: Some(
                    r#"{"nickname": "新昵称", "avatar": "https://example.com/avatar.jpg"}"#
                        .to_string(),
                ),
                timeout: Some(30),
                is_history: false,
        ...
        let update_profile_node = tree_dao
            .create_request_node(CreateRequestNodeRequest {
        ...
        assert_eq!(
            update_profile_node.api_debug_id,
            Some(update_profile_api.id)
        );
