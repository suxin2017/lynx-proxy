...
const HexViewer: React.FC<HexViewerProps> = ({ data }) => {
    ...
    const renderRows = () => {
        ...
        for (let i = 0; i < data.length; i += 16) {
            ...
            const asciiText = hexBytes.map(formatAscii).join('');

            rows.push(
                <div key={i} className="flex items-center text-sm">
                    <span className="w-[8ch] text-right text-gray-500 mr-4">{offset}</span>
                    {hexBytes.map((byte, idx) => {
                        ...
                        return (
                            <span
                                key={idx}
                                className={`w-6 text-center cursor-pointer ${selected ? 'bg-blue-200' : 'text-gray-800'
                                    }`}
